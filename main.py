...
class Admin:
    def __init__(self) -> None:
        ...
        self.thr_presets = {
            ...
            's': {'target': self.scheduler.schedule, "args": []}
        }
        self.threads = {}
    ...
    def start(self, *threads) -> None:
        ...
        for thr in threads:
            try:
                if getattr(self, 'bot') is None:
                    self.bot = GeorgeBot(getattr(self, 'token'), self.users_data, self.orders_data)
                if getattr(self, 'scheduler') is None:
                    self.scheduler = Scheduler(self.bot)
                arguments = self.thr_presets[thr]
    ...
    def __main(self, bot: GeorgeBot, users_data: Base, orders_data: Base):
        ...
        def calendary(call: telebot.types.CallbackQuery):
            ...
            if hour is None:
                user.update({"timeline": bot.available_hours(user)})
            ...
            if minute is None:
                user.update({"timeline": bot.available_minutes(user)})
                bot.display(user, "calendary_minuts")
                return
            if 0 > minute > 60:
                return
        ...
        def content_handler(message: telebot.types.Message):
            ...
            match message.content_type:
                case "text":
                    bot.text_handler(message, user, path_end)
                case "document" | "audio":
